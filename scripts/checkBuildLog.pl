...
#		print "INSPECT: $_";
		...
		if (/- 0 error\(s\),|\w table successfully built|Dialog [^ ]+ for language\s+built/)
		{
			next loop;
		}
		...
		# Exclude BlackBox error
		if (/[\/_]error.html/)
		{
			next loop;
		}
		
		# Exclude Desktop Scanner error
		if (/warning C4189: .szError. : local variable/)
		{
			next loop;
		}
